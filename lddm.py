...
parser.add_argument('-t',action='store_true',default=False,dest='text',help='only print the result with text instead of plot')
...
#
#
if not args.text: ax2.imshow(data0[::-1],aspect='auto',interpolation='nearest',extent=(0,1,freq_start0,freq_end0),cmap='jet')
if args.frequency:
...
def shift(y,x):
	...
	return fftr
#
psr=pm.psr_timing(pr.psr(info['psr_par']),te.times(te.time(np.float64(info['stt_time'])+np.float64(info['length'])/86400,0)),freq.mean())
...
if args.n:
	...
	order=7
#
dmmax,dmerr,dm,value,fitvalue=af.dmdet(fftdata,const,ddm,zone,order)
#
if not args.text:
	x0=dm.min()+dm0
	x1=dm.max()+dm0
...
if dmerr>0:
	if not args.text:
		...
		tmp=np.shape(fftdata)[-1]
		frac=1/(freq*psr.vchange)**2*4148.808/period*dmmax
		...
		print('DM_0='+str(dm0)+'\nBest DM='+str(np.round(dmmax+dm0,ndigit))+'+-'+str(np.round(dmerr,ndigit)))
else:
	...
	else: print('The best DM cannot be found.')
#
if not args.text:
